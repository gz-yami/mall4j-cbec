...
public class UserCollectionController {
    ...
    public ServerResponseEntity<Boolean> isCollection(String prodId) {
        ...
        if (ArrayUtil.isEmpty(prodId)) {
            // 商品id不能为空
            throw new YamiShopBindException("yami.user.collection.exception.prodIdNull");
        }
        int count = 0;
    ...
    @Operation(summary = "查询用户收藏数量(商品收藏数量)" , description = "查询用户收藏数量")
    public ServerResponseEntity<Long> userCollectionCount() {
        ...
        return ServerResponseEntity.success(prodCollectionCount);
    }
    ...
    public ServerResponseEntity<IPage<ProductDto>> collectionProds(PageParam page) {
        String userId = SecurityUtils.getUser().getUserId();
        IPage<ProductDto> prodPage  = userCollectionService.collectionProds(page, userId);
