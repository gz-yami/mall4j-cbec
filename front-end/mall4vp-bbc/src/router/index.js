...
export const mainRoutes = {
  ...
  beforeEnter (to, from, next) {
    const authorization = cookie.get('cbecB2cAuthorization_vp')
    if (!authorization || !/\S/.test(authorization)) {
      clearLoginInfo()
      next({ name: 'login' })
    }
    useRouterStore().updateIncludePageList(to.name)
    useRouterStore().pushHisPageList(to.name)
    next()
  }
}
...
router.beforeEach((to, from, next) => {
  ...
  if (router.options.isAddDynamicMenuRoutes || fnCurrentRouteType(to, globalRoutes) === 'global') {
    ...
    http({
      ...
      sessionStorage.setItem('cbecB2cAuthorities', JSON.stringify(data.authorities || '[]'))
      ...
      fnAddDynamicMenuRoutes(data.menuList)
      sessionStorage.setItem('cbecB2cMenuList', JSON.stringify(data.menuList || '[]'))
