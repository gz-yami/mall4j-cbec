...
export default {
  ...
  setup(props) {
    ...
    const uuid = () => {
      ...
      // 判断下是否存在 slider
      if (!localStorage.getItem('cbecB2cSlider')) {
        localStorage.setItem('cbecB2cSlider', slider)
      }
      if (!localStorage.getItem('cbecB2cPoint')) {
        localStorage.setItem('cbecB2cPoint', point)
      }
    }
    const show = () => {
      if (mode.value === 'pop') {
