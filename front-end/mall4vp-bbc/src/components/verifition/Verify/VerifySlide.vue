...
export default {
  ...
  props: {
    ...
    explain: {
      type: String,
      default: $t('swipeRightToCompleteVerification')
    },
    imgSize: {
      type: Object,
      default () {
  ...
  setup (props) {
      ...
      if (status.value && isEnd.value === false) {
        ...
        reqCheck(data).then(res => {
          if (res.data.repCode === '0000') {
            ...
            passFlag.value = true
            tipWords.value = `${((endMovetime.value - startMoveTime.value) / 1000).toFixed(2)}s${$t('verifySuccessfully')}`
            ...
            tipWords.value = $t('verificationFailure')
            setTimeout(() => {
              tipWords.value = ''
            }, 1000)
          }
    ...
    function getPictrue () {
      const data = {
        captchaType: captchaType.value,
        clientUid: localStorage.getItem('cbecB2cSlider'),
        ts: Date.now() // 现在的时间戳
      }
